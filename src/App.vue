<template>
  <div class="h-full">
    <div
      class="app-container h-full transition-colors duration-300 dark:bg-dark-background bg-white"
    >
      <!-- Navbar -->
      <nav class="navbar">
        ...
        <div class="navbar-right">
          ...
          <div class="auto-toggle-wrapper">
            <div class="auto-toggle-label dark:text-dark-text text-gray-800">Auto Detection</div>
            <button
      ...
      <div class="frequency-display">
        <div class="frequency">{{ currentFrequency.toFixed(1) }}<span>Hz</span></div>
      ...
      <div
        v-if="!isAutoMode"
        class="note-buttons fixed bottom-8 left-1/2 transform -translate-x-1/2 flex justify-center gap-1 w-full max-w-xl px-2"
      >
        <button
          ...
          :class="{
            'ring-2 ring-primary':
              currentNote?.name === note.name && currentNote?.octave === note.octave,
          }"
        ...
        >
          <div class="font-semibold text-sm">
            {{ note.name }}{{ note.sharp ? '#' : '' }}{{ note.octave }}
          </div>
      ...
      <div class="footer-attribution">
        <span
          >Based on
          <a href="https://github.com/qiuxiang/tuner" target="_blank" rel="noopener noreferrer"
            >@qiuxiang/tuner</a
          >
          and changed by
          <a href="https://guisso.dev" target="_blank" rel="noopener noreferrer">guisso</a>.</span
        >
      </div>
    </div>
  </div>
...
import { FrequencyBars } from './utils/frequency-bars'
import { createNotes, getFrequencyFromNoteValue } from './utils/notes'
  ...
  setup() {
    ...
    const selectNote = (note) => {
      ...
      if (Array.isArray(notesList.value)) {
        ...
        const completeNote = notesList.value.find(
